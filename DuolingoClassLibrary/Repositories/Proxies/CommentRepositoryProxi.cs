...
public class CommentRepositoryProxi : ICommentRepository, IDisposable
{
    ...
    {
        var response = await _httpClient.GetAsync(Enviroment.BaseUrl + $"comment/{commentId}");
    ...
    {
        var response = await _httpClient.GetAsync(Enviroment.BaseUrl + $"comment/ByPost/{postId}");
        if (!response.IsSuccessStatusCode)
    ...
    public async Task<int> CreateComment(Comment comment)
    {
        ...
        var response = await _httpClient.PostAsync(Enviroment.BaseUrl + "comment", jsonContent);
        if (!response.IsSuccessStatusCode)
    ...
    {
        var response = await _httpClient.DeleteAsync(Enviroment.BaseUrl + $"comment/{id}");
        if (!response.IsSuccessStatusCode)
    ...
    {
        var response = await _httpClient.GetAsync(Enviroment.BaseUrl + $"comment/Replies/{parentCommentId}");
        if (!response.IsSuccessStatusCode)
    ...
    {
        var response = await _httpClient.PostAsync(Enviroment.BaseUrl + $"comment/Like/{commentId}", null);
        if (!response.IsSuccessStatusCode)
    ...
    {
        var response = await _httpClient.GetAsync(Enviroment.BaseUrl + $"comment/Count/{postId}");
        if (!response.IsSuccessStatusCode)
